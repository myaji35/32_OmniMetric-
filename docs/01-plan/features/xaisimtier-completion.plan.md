...
| FR-13 | B2B 데이터 커넥터 (갑-을 연동, API Key 관리, 멀티테넌시) | P1-High | Pending |
...
### Phase 7: B2B 데이터 커넥터 (P1)
**예상 소요: 1일**

17. **B2B 데이터 커넥터** (FR-13)
    - `app/services/connector.py` - B2BConnectorService
    - `app/api/v1/endpoints/connectors.py` - 7개 엔드포인트
    - API Key 발행/검증/폐기 (SHA-256 해시 저장)
    - 고객사별 데이터 격리 (멀티테넌시)
    - 데이터 수집 스케줄링 (실시간/주기적/요청시)
    - 스키마 자동 탐색
    - 감사 로그 (누가, 언제, 어떤 데이터 수집)
    - IP 화이트리스트, Rate Limiting per Key
...
| 0.2 | 2026-03-12 | B2B 데이터 커넥터 Phase 7 추가, FR-13 추가 | CTO Lead Agent |
